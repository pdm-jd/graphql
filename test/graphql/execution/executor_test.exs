defmodule GraphQL.Execution.Executor.ExecutorTest do
  ...
  test "TypeChecked fragments run the correct type" do
    ...
    {:ok, result} =
      execute(
        schema,
        ~S[{id, ...spreada ...spreadb} fragment spreadb on BType { b } fragment spreada on AType { a }]
      )
  ...
  test "must specify operation name when multiple operations exist" do
    {_, result} =
      execute(TestSchema.schema(), "query a {greeting} query b {greeting} query c {greeting}")
  ...
  test "Quoted fields are available" do
    {:ok, result} =
      execute(TestSchema.recursive_schema(), "{id, b { name, c{ id, name, b { name }}}}")

    assert_data(result, %{
      id: "1",
      b: %{name: "Mark", c: %{id: "2", name: "Kate", b: %{name: "Mark"}}}
    })
  end
  ...
  test "use specified query operation" do
    ...
    {:ok, result} =
      execute(schema, ~S[query Q { a } mutation M { b }], root_value: data, operation_name: "Q")
  ...
  test "use specified mutation operation" do
    ...
    {:ok, result} =
      execute(schema, ~S[query Q { a } mutation M { b }], root_value: data, operation_name: "M")
