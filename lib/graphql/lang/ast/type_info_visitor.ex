      ...
      accumulator =
        case node.kind do
          ...
          kind when kind in [:InlineFragment, :FragmentDefinition] ->
            output_type =
              if Map.has_key?(node, :typeCondition) do
          ...
          :Argument ->
            field_or_directive =
              TypeInfo.directive(accumulator[:type_info]) ||
            ...
            if field_or_directive do
              ...
              stack_push(
                accumulator,
                :input_type_stack,
                if(arg_def && Map.has_key?(arg_def, :type), do: arg_def.type, else: nil)
              )
            else
