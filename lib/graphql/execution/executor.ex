defmodule GraphQL.Execution.Executor do
  ...
  Execute a query against a schema.
      ...
      # {:ok, %{hello: world}}
  """
  @spec execute(GraphQL.Schema.t(), GraphQL.Document.t(), list) ::
          result_data | {:error, %{errors: list}}
  ...
  defp expand_options(opts) do
    {Keyword.get(opts, :root_value, %{}), Keyword.get(opts, :variable_values, %{}),
     Keyword.get(opts, :operation_name, nil)}
  end

  @spec execute_operation(ExecutionContext.t(), Nodes.operation_node(), map) ::
          result_data | {:error, String.t()}
  defp execute_operation(context, operation, root_value) do
    type = Schema.operation_root_type(context.schema, operation)

    {context, %{fields: fields}} =
      Selection.collect_selections(context, type, operation.selectionSet)
