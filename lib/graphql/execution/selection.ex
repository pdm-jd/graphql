defmodule GraphQL.Execution.Selection do
  ...
  @spec execute_fields_serially(ExecutionContext.t(), atom, map, any) ::
          {ExecutionContext.t(), map}
  ...
  def execute_fields(context, parent_type, source_value, fields) do
    Enum.reduce(fields, {context, %{}}, fn {field_name_ast, field_asts}, {context, results} ->
      FieldResolver.resolve_field(
        context,
        Types.unwrap_type(parent_type),
        source_value,
        field_asts
      )
  ...
  def collect_sub_fields(context, return_type, field_asts) do
    Enum.reduce(field_asts, {context, %{fields: %{}, fragments: %{}}}, fn field_ast,
                                                                          {context,
                                                                           field_fragment_map} ->
  ...
  def collect_selections(
        ...
        runtime_type,
        selection_set,
        field_fragment_map \\ %{fields: %{}, fragments: %{}}
      ) do
    Enum.reduce(selection_set[:selections], {context, field_fragment_map}, fn selection,
                                                                              {context,
                                                                               field_fragment_map} ->
  ...
  defp typecondition_matches?(context, selection, runtime_type) do
    ...
    cond do
      ...
      condition_ast.name.value == runtime_type.name ->
        true

      # the type doesn't exist, so, it can't match
      typed_condition == :not_found ->
        false
