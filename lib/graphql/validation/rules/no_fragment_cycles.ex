  ...
  defimpl Visitor do
    ...
    defp process_one_node(acc, spread_node, cycle_index) when is_integer(cycle_index) do
      ...
      report_error(
        acc,
        cycle_error_message(
          spread_node.name.value,
          Enum.map(cycle_path, fn s -> s.name.value end)
        )
      )
    end
    ...
    defp process_one_node(acc, spread_node, _) do
      ...
      acc =
        if !visited?(acc, spread_node) do
          spread_fragment =
            DocumentInfo.get_fragment_definition(acc[:document_info], spread_node.name.value)
    ...
    defp mark_visited(acc, node) do
      %{acc | visited_fragments: Map.merge(acc[:visited_fragments], %{node.name.value => true})}
    end

    defp cycle_error_message(frag_name, spread_names) do
      via =
        case length(spread_names) do
          0 -> ""
